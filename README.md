...
1. **Host application** (`app run`, `internal/app/`) - Runs outside the enclave, handles validator requests, and persists the wrapped key in AWS Secrets Manager. During startup, it passes temporary AWS credentials, KMS key ARNs, the LocalStack selector, and either the stored envelope or a key-generation request to enclave `Initialize`. It does not construct KMS requests or handle plaintext data keys.
...
3. **Enclave application** (`internal/enclave/`) - Runs inside an AWS Nitro Enclave. Each `Initialize` attempt creates an ephemeral AWS KMS client, attaches NSM-signed attestation to KMS requests, and routes them through the enclave-side `awsproxy`. A successful attempt installs one signing key in enclave memory. Later public-key and signing requests use that key without KMS.
...
The host application calls this API over TCP in dev/CI or VSOCK in Nitro
production. Arc Chain validators do not interact with this API directly.
`Initialize` generates or recovers the signing key and returns its public key
and attestation. `GetPublicKey` refreshes the public-key attestation.
`SignMessage` receives only the message. `GenerateKey` is retained for protocol
compatibility and rejects direct key generation.
...
The internal `Initialize` protocol changed in v1.7.7. Upgrade or roll back the
host and enclave as one unit. A mixed-version host and enclave are not
compatible.

If a restarted host receives a stale replayed attestation from a long-lived
enclave, restart the enclave with the host. Use this procedure until upstream
re-attestation behavior is fixed.
...
Circle publishes the tested Arc Remote Signer image to
`docker.cloudsmith.io/circle/arc/arc-remote-signer`. Circle sends the Cloudsmith
entitlement token to each authorized partner in a secure email. Circle sends
the versioned image reference in the partner Slack channel. The operator must
pull the supplied image. The operator must record the image digest. The image
labels contain PCR0, PCR1, and PCR2. The KMS policy in this repository requires
PCR0, PCR1, PCR2, and PCR4 (derived from the parent EC2 instance ID). The
operator must use the PCR values from the pulled image and parent instance. The
operator does not have to build the Circle-published image.

Circle syncs the source code, but not the Circle-published image, to the public
`circlefin/arc-remote-signer` repository.

An operator can build a custom image from the public source code instead of
using the Circle-published image. The operator controls the build environment.
The operator configures the KMS key policy with the measured PCR0, PCR1, and
PCR2 values and the parent instance PCR4 value. A reproducible build permits
independent verification, but it does not establish trust in the build
environment.

A custom image is not the Circle-published Arc Remote Signer artifact. The
operator is responsible for the integrity of the custom build environment.
...
the tested image to Cloudsmith and ECR. Operators must pull the supplied image
reference. Operators must then record the image digest. Operators must use the
recorded image digest in the deployment configuration. Staging and production
must use the recorded digest.
