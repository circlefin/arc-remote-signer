...
type testNestedConfig struct {
	Methods map[string]testMethodConfig
}
...
func TestLoadConfig_EnvironmentOverridesNestedMapValue(t *testing.T) {
	t.Setenv("APP_NESTED_METHODS_INITIALIZE_TIMEOUTMS", "123")

	cfg := newTestAppConfig()
	LoadConfig(cfg, "")

	require.Len(t, cfg.Nested.Methods, 1)
	require.Equal(t, 123, cfg.Nested.Methods["initialize"].TimeoutMS)
}
