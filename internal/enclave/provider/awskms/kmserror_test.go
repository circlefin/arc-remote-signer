...
func TestStatusFromError(t *testing.T) {
	tests := map[string]struct {
		...
		"ExpiredToken": {
			err:        &smithy.GenericAPIError{Code: "ExpiredTokenException", Message: "security token expired"},
			wantCode:   codes.Unauthenticated,
			wantMapped: true,
		},
		"InvalidClientTokenID": {
			err:        &smithy.GenericAPIError{Code: "InvalidClientTokenId", Message: "access key does not exist"},
			wantCode:   codes.Unauthenticated,
			wantMapped: true,
		},
		"UnrecognizedClient": {
			err:        &smithy.GenericAPIError{Code: "UnrecognizedClientException", Message: "invalid credentials"},
			wantCode:   codes.Unauthenticated,
			wantMapped: true,
		},
		"Throttling": {
