...
func TestNewProvider_AttestsRetainedRSAKey(t *testing.T) {
	privateKey, err := rsa.GenerateKey(rand.Reader, rsaKeyBits)
	require.NoError(t, err)
	var gotRequest *request.Attestation
	session := &testNSMSession{
		send: func(req request.Request) (response.Response, error) {
			gotRequest = req.(*request.Attestation)
			return response.Response{
				Attestation: &response.Attestation{Document: []byte("attestation-document")},
			}, nil
		},
	}

	gotProvider, err := newProvider(func() (nsmSession, error) { return session, nil }, privateKey)

	require.NoError(t, err)
	require.Equal(t, []byte("attestation-document"), gotProvider.AttestationDocument())
	require.True(t, session.closed)
	require.NotNil(t, gotRequest)
	parsedPublicKey, err := x509.ParsePKIXPublicKey(gotRequest.PublicKey)
	require.NoError(t, err)
	require.Equal(t, &privateKey.PublicKey, parsedPublicKey)
}

func TestNewProvider_RejectsInvalidAttestationResponse(t *testing.T) {
	privateKey, err := rsa.GenerateKey(rand.Reader, rsaKeyBits)
	require.NoError(t, err)
	tests := []struct {
		name     string
