...
func run(cfg *Config, nitroEnabled bool, newAWSProxy awsProxyFactory) error {
	...
	awskmsFactory := func(callCtx context.Context, awsCfg aws.Config, arns []string) (awskms.Provider, error) {
		...
		if nitroEnabled {
			return awskms.NewWithAttestation(callCtx, kmsConfig, awsCfg, enclavePvd.AttestationDocument())
		}
