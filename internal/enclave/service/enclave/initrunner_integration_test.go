...
func TestNewRunInit_Integration_HappyPath(t *testing.T) {
	s := newServiceForTest(t, false, nil, newLocalstackFactory(t), localstackEndpoint)
	_, err := s.runInit(context.Background(), validIntegrationReq(t, localstackKmsArn))
...
func TestNewRunInit_Integration_InvalidArn(t *testing.T) {
	s := newServiceForTest(t, false, nil, newLocalstackFactory(t), localstackEndpoint)
	_, err := s.runInit(context.Background(), validIntegrationReq(t, localstackInvalidArn))
...
func TestNewRunInit_Integration_InvalidEndpoint(t *testing.T) {
	s := newServiceForTest(t, false, nil, newLocalstackFactory(t), "http://127.0.0.1:65535")
	_, err := s.runInit(context.Background(), validIntegrationReq(t, localstackKmsArn))
...
func TestService_Envelope_Integration_GenerateThenSign(t *testing.T) {
	...
	s := newServiceForTest(t, false, nil, newLocalstackFactory(t), localstackEndpoint)
	initResp, err := s.Initialize(ctx, validIntegrationReq(t, localstackKmsArn))
	require.NoError(t, err, "Initialize must succeed against LocalStack")

	msg := []byte("integration-message")
	sig, err := s.SignMessage(ctx, &pb.SignMessageRequest{Message: msg})
	require.NoError(t, err)
	require.NotEmpty(t, sig.Signature)

	ok, err := crypto.VerifySignedMessage(crypto.AlgorithmEd25519, sig.Signature, msg, initResp.PublicKey)
	require.NoError(t, err)
