...
func TestProductionConfig_PreservesEnclaveRequestTimeout(t *testing.T) {
	...
	require.Equal(t, 30000, cfg.Provider.Enclave.Client.Methods["initialize"].TimeoutMS)
	require.Equal(t, uint(1), cfg.Provider.Enclave.Client.Methods["initialize"].MaxAttempts)
	require.Equal(t, 30000, cfg.Provider.Enclave.Client.Methods["generatekey"].TimeoutMS)
	require.Equal(t, uint(1), cfg.Provider.Enclave.Client.Methods["generatekey"].MaxAttempts)
	require.Equal(t, 30000, cfg.Provider.Enclave.Client.Methods["getpublickey"].TimeoutMS)
	require.Equal(t, 500, cfg.Provider.Enclave.Client.Methods["signmessage"].TimeoutMS)
	require.Equal(t, uint(1), cfg.Provider.Enclave.Client.Methods["signmessage"].MaxAttempts)
	require.Equal(t, 30000, cfg.Provider.Enclave.StartupTimeoutMS)
	require.Equal(t, 30*time.Second, cfg.Provider.Enclave.StartupTimeout())
}

func TestProductionConfig_MethodTimeoutEnvironmentOverride(t *testing.T) {
	t.Setenv("APP_PROVIDER_ENCLAVE_CLIENT_METHODS_INITIALIZE_TIMEOUTMS", "1234")
	cfg := NewConfig()

	config.LoadConfig(cfg, filepath.Join("..", "..", "configs", "app.production.yaml"))

	require.Equal(t, 1234, cfg.Provider.Enclave.Client.Methods["initialize"].TimeoutMS)
}
