...
// Run constructs the vsockproxy, registers it with a lifecycle manager,
// and blocks on Manager.Run until SIGINT or SIGTERM. The transport
// option is derived from cfg.Provider.Enclave.NitroEnclave.Enabled, the
// same toggle the app process consumes.
//
// Returns an error only if New fails; the cobra subcommand surfaces it
// via RunE (exit 1). lifecycle.Manager.Run log.Fatals on runtime
// runnable failures, which is fine for a single-runnable process.
func Run(cfg *Config) error {
	applyBuildPolicy(cfg)

	logger := logging.Get(configName)
	logger.Info(context.Background(), "starting vsockproxy", logging.Entries{
		"nitroEnclaveEnabled": cfg.Provider.Enclave.NitroEnclave.Enabled,
		"basePort":            cfg.Vsockproxy.BasePort,
	})

	p, err := New(cfg, runtimeTransportOption(cfg))
	if err != nil {
		return err
	}
